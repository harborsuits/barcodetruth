-- Fix 1: Update get_power_profit RPC to include canonical role names
...
BEGIN
  ...
  -- Get leadership (FIXED: include canonical role names)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'name', person_name,
...
-- Fix 2: Remove John C. Bogle from companies he didn't found
-- He only founded The Vanguard Group, not J&J, McDonald's, CVS, Starbucks, etc.
DELETE FROM company_people 
WHERE person_name = 'John C. Bogle' 
  AND company_id NOT IN (
    SELECT id FROM companies WHERE name ILIKE '%vanguard%'
  );

-- Fix 3: Backfill tickers from brands to companies where missing
UPDATE companies c
SET ticker = b.ticker
FROM brands b
WHERE c.wikidata_qid = b.wikidata_qid
