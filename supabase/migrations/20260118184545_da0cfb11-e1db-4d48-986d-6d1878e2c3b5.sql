...
-- Fix get_power_profit RPC with correct column names for company_people
...
BEGIN
  ...
  -- Get leadership (using person_name, not name)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'name', person_name,
        'role', role,
        'title', role,  -- Use role as title since no title column exists
        'image_url', image_url,
  ...
  -- Get parent company (using child_brand_id)
