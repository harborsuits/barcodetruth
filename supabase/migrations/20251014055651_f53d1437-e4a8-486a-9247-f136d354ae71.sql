-- TICKET C: Brand profile view RPC
-- Returns comprehensive brand data: brand info, score, coverage, and latest evidence
...
AS $$
  WITH b AS (
    SELECT id, name, parent_company, website
    FROM brands 
    WHERE id = p_brand_id
  ),
  s AS (
  ...
  -- Get 30-day events count manually since brand_data_coverage doesn't have it
  events_30d_calc AS (
    SELECT COUNT(*) AS cnt
    FROM brand_events
    WHERE brand_id = p_brand_id
