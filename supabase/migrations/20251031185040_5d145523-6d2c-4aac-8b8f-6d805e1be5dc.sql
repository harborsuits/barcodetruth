-- Fix NULL name constraint violation by skipping invalid staging rows
...
BEGIN
  ...
  WITH cand AS (
    ...
    WHERE COALESCE(s.brand_label, '') <> ''
      AND s.product_name IS NOT NULL  -- ✅ Skip NULL names
      AND s.barcode IS NOT NULL        -- ✅ Skip NULL barcodes
  ), missing AS (
    SELECT c.norm
    FROM cand c
  ...
  WITH normed AS (
    ...
    FROM staging_products s
    WHERE s.product_name IS NOT NULL  -- ✅ CRITICAL: Skip NULL names
      AND s.barcode IS NOT NULL       -- ✅ CRITICAL: Skip NULL barcodes
    ORDER BY s.id
    LIMIT batch_size
  ),
  resolved AS (
  ...
  SELECT
    ...
    GREATEST((SELECT COUNT(*)::int FROM staging_products) - batch_size, 0) AS remaining,
