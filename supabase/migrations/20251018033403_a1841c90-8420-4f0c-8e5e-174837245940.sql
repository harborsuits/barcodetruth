
-- Fix brand_profile_view function to use correct column names
CREATE OR REPLACE FUNCTION brand_profile_view(p_brand_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = 'public'
AS $$
  WITH b AS (
    SELECT id, name, parent_company, website, description, description_source, logo_url, logo_attribution
    FROM brands 
    WHERE id = p_brand_id
  ),
  s AS (
    SELECT 
      score AS score,
      last_updated AS updated_at,
      breakdown AS reason_json
    FROM brand_scores 
    WHERE brand_id = p_brand_id
    ORDER BY last_updated DESC
    LIMIT 1
  ),
  c AS (
    SELECT 
      COALESCE(events_30d, 0) AS events_30d,
      COALESCE(events_365d, 0) AS events_365d,
      COALESCE(verified_rate, 0) AS verified_rate,
      COALESCE(independent_sources, 0) AS independent_sources,
      last_event_at
    FROM brand_data_coverage 
    WHERE brand_id = p_brand_id
  ),
  e AS (
    SELECT
      be.event_date,
      COALESCE(be.title, 'Untitled event') AS title,
      be.verification::text AS verification,
      es.source_name,
      es.canonical_url,
      be.category::text AS category
    FROM brand_events be
    LEFT JOIN event_sources es ON es.event_id = be.event_id
    WHERE be.brand_id = p_brand_id
    ORDER BY be.event_date DESC NULLS LAST
    LIMIT 25
  )
  SELECT jsonb_build_object(
    'brand', (SELECT COALESCE(to_jsonb(b.*), 'null'::jsonb) FROM b),
    'score', (SELECT COALESCE(to_jsonb(s.*), 'null'::jsonb) FROM s),
    'coverage', (
      SELECT jsonb_build_object(
        'events_30d', COALESCE(c.events_30d, 0),
        'events_365d', COALESCE(c.events_365d, 0),
        'verified_rate', COALESCE(c.verified_rate, 0),
        'independent_sources', COALESCE(c.independent_sources, 0),
        'last_event_at', c.last_event_at
      )
      FROM c
    ),
    'evidence', (SELECT COALESCE(jsonb_agg(to_jsonb(e.*)), '[]'::jsonb) FROM e)
  );
$$;

-- Create personalized_brand_score function
CREATE OR REPLACE FUNCTION personalized_brand_score(p_brand_id uuid, p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path = 'public'
AS $$
DECLARE
  v_result jsonb;
BEGIN
  -- Get brand score with user preferences
  SELECT jsonb_build_object(
    'score', COALESCE(bs.score, 50),
    'score_labor', COALESCE(bs.score_labor, 50),
    'score_environment', COALESCE(bs.score_environment, 50),
    'score_politics', COALESCE(bs.score_politics, 50),
    'score_social', COALESCE(bs.score_social, 50),
    'breakdown', bs.breakdown,
    'last_updated', bs.last_updated
  ) INTO v_result
  FROM brand_scores bs
  WHERE bs.brand_id = p_brand_id
  ORDER BY bs.last_updated DESC
  LIMIT 1;
  
  -- If no score exists, return baseline
  IF v_result IS NULL THEN
    v_result := jsonb_build_object(
      'score', 50,
      'score_labor', 50,
      'score_environment', 50,
      'score_politics', 50,
      'score_social', 50,
      'breakdown', NULL,
      'last_updated', NULL
    );
  END IF;
  
  RETURN v_result;
END;
$$;
