...
BEGIN
  ...
  better_opts AS (
    SELECT cd.*, 'better'::text AS group_label,
      ROW_NUMBER() OVER (ORDER BY cd.category_bonus DESC, (COALESCE(cd.base_score, 50) + cd.independence_bonus) DESC) AS rn
    FROM candidates cd
    WHERE cd.is_better_eligible = true
  ),
  similar_opts AS (
    SELECT cd.*, 'similar'::text AS group_label,
      ROW_NUMBER() OVER (ORDER BY cd.category_bonus DESC, COALESCE(cd.base_score, 50) DESC) AS rn
    FROM candidates cd
  ...
  SELECT
    ...
    CASE
      ...
      WHEN cd.group_label = 'better' AND cd.base_score >= 70 THEN 'Higher overall ethics score'
      WHEN cd.group_label = 'better' THEN 'Better option in this category'
      ELSE 'Similar product'
    END AS reason,
    ROUND((COALESCE(cd.base_score, 50) + cd.independence_bonus + cd.category_bonus)::numeric, 1) AS score,
  ...
  ORDER BY
    ...
    (COALESCE(cd.base_score, 50) + cd.independence_bonus) DESC;
END;
$function$;
