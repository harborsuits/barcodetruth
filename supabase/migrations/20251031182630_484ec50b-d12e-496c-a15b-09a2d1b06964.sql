-- Drop old function signature (without dry_run parameter)
DROP FUNCTION IF EXISTS public.merge_staged_products_batch(integer);

-- Recreate the function with both parameters (ensures clean state)
...
DECLARE
  v_created int := 0;
BEGIN
  -- Create any missing brands from current staging batch
  WITH cand AS (
