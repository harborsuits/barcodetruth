-- Add deduplication to handle duplicate barcodes in staging
...
BEGIN
  ...
  -- ✅ CRITICAL FIX: Deduplicate by barcode (keep first occurrence)
  deduped AS (
  ...
  to_merge AS (
    SELECT * FROM deduped WHERE brand_id IS NOT NULL
  ),
  unmapped AS (
    SELECT * FROM deduped WHERE brand_id IS NULL
  ),
  upserted AS (
