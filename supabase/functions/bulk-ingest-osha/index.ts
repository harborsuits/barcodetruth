...
Deno.serve(async (req) => {
  ...
  const _gate = await requireAdminOrInternal(req, "bulk-ingest-osha"); if (_gate) return _gate;
);
  }
  ...
  try {
    ...
    console.log("[bulk-ingest-osha] Starting bulk OSHA ingestion...");
    ...
    console.log(`[bulk-ingest-osha] Processing ${pilots.length} pilot brands`);
    ...
    for (const pilot of pilots) {
      ...
      try {
        const ingestUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/fetch-osha-events?brand_id=${brandId}`;
        ...
        console.log(`[bulk-ingest-osha] ${brandName}: ${data.inserted || 0} new events`);

        results.push({ brand_id: brandId, name: brandName, success: true });
      } catch (error: any) {
        console.error(`[bulk-ingest-osha] Error for ${brandName}:`, error);
    ...
    console.log(`[bulk-ingest-osha] Complete: ${successful}/${results.length} succeeded`);
    ...
    console.error("[bulk-ingest-osha] Fatal error:", error);
    return new Response(
