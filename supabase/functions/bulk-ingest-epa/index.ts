...
Deno.serve(async (req) => {
  ...
  const _gate = await requireAdminOrInternal(req, "bulk-ingest-epa"); if (_gate) return _gate;
);
  }
  ...
  try {
    ...
    console.log("[bulk-ingest-epa] Starting bulk EPA ingestion...");
    ...
    console.log(`[bulk-ingest-epa] Processing ${pilots.length} pilot brands`);
    ...
    for (const pilot of pilots) {
      ...
      try {
        const ingestUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/fetch-epa-events?brand_id=${brandId}`;
        ...
        console.log(`[bulk-ingest-epa] ${brandName}: ${data.inserted || 0} new events`);

        results.push({ brand_id: brandId, name: brandName, success: true });
      } catch (error: any) {
        console.error(`[bulk-ingest-epa] Error for ${brandName}:`, error);
    ...
    console.log(`[bulk-ingest-epa] Complete: ${successful}/${results.length} succeeded`);
    ...
    console.error("[bulk-ingest-epa] Fatal error:", error);
    return new Response(
