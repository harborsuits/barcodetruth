// Public cron wrapper: invokes internal brand-match with INTERNAL_FN_TOKEN
...
Deno.serve(async (req) => {
  ...
  try {
    const { data, error } = await supabase.functions.invoke('brand-match', {
      body: {},
    ...
    console.error('cron-brand-match error', err)
